...
class InfluxDBPublicationChannel extends PublicationChannel {
  ...
  publish(data) {
    ...
    return Promise.resolve(data).then( (status) => {
      ...
      if (status.dailyGasUsage && status.dailyGasUsage.heating && status.dailyGasUsage.hotwater && status.dailyGasUsage.averageOutdoorTemperature && status.dailyGasUsage.timestamp) {
        dataPoints.push({
    ...
    return Promise.resolve(history).then( (history) => {
      ...
      if (history.gasUsage) {
        history.gasUsage.forEach( (gasUsageRecord ) => {
          if (gasUsageRecord && gasUsageRecord.heating && gasUsageRecord.hotwater && gasUsageRecord.averageOutdoorTemperature && gasUsageRecord.timestamp) {
            dataPoints.push({
              measurement: 'nefit_easy_daily_gas_usage',
